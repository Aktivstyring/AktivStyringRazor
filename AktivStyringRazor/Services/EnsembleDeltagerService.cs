...
namespace AktivStyringRazor.Services
{
    public class EnsembleDeltagerService : Connection, IEnsembleDeltagerService
    {
        ...
        private string queryById = "select * from EnsembleDeltagere where EnsDeltager = @ID";
        private string insertSql = "insert into EnsembleDeltagere(Rolle) values(@Rolle)";
        private string queryDelete = "delete from EnsembleDeltagere where EnsDeltager = @ID";
        ...
        public async Task<bool> AddRollerAsync(Roller roller)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                command.Parameters.AddWithValue("@Roller", roller.Rolle);
